...
public class StoredXss extends LessonAdapter
{
	...
	protected void addMessage( WebSession s )
	{
		try
		{
			String title = Encoding.htmlEscape( s.getParser().getRawParameter( TITLE, "" ) );
			...
			// This query is subject to SQL injection.  You could bypass the user name and insert
			// another users user name into this field.
			String query = "INSERT INTO messages VALUES (" + count++ + ",'" + title + "','" + message + "','" + s.getUserName() + "')";
	...
	{
		return makeMenuItem( "Stored XSS", WebSession.SCREEN, getScreenId() );
	}
	...
	protected Element makeCurrent( WebSession s )
	{
		...
		try
		{
			...
			String query = "SELECT * FROM messages WHERE user_name LIKE '" + getNameroot( s.getUserName() ) + "%' and num = " + messageNum;
